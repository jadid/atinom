...
void hapus_layar(unsigned short x, unsigned short y, unsigned short x2, unsigned short y2)
{
	//15 des 06, menghapus layar sesuai dengan koordinat
	int i;
	int j;
   	...
   	for (i=y; i<y2; i++)
	{
   		...
   		for (j=x; j<x2;)
   		{
			...
			/* disempurnakan, menghapus sisanya */   		
   			if (hasil.rem > 0)
   			{
   				temp = 0xFF;
   				layar.buf[offsetku] = layar.buf[offsetku] & (~(temp >> hasil.rem));
			}
